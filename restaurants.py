...
def print_restaurant_data(postcode, sort_by):
    ...
    if restaurants:
        ...
        for restaurant in restaurants_filtered[:10]:  
            ...
            restaurant_name = restaurant.get('name', 'Name not available.')   
            ...
            cuisine = ', '.join(cuisine['name'] for cuisine in cuisine_data) if cuisine_data else 'Cuisine not available.'
            
            #extracting the rating 
            rating_info = restaurant.get('rating', {})
            rating = f"{rating_info.get('starRating', 'Rating not available.')} ({rating_info.get('count', 0)} reviews)"
            
            #extracting the address 
            address_info = restaurant.get('address', {})
            address = f"{address_info.get('firstLine', 'Address not available.')}, {address_info.get('city', 'City not available.')}, {address_info.get('postalCode', 'Post code not available.')}"
...
if __name__ == '__main__':
    ...
    if not postcode:
        ...
        if sort_by_choice == "1":
            ...
            print("Invalid choice. Displaying restaurants randomly.")
            sort_by = "random"
